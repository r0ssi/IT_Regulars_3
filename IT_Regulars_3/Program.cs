...
Regex wordList = new Regex(@"^(?=.*\bдыня\b)(?=.*\bарбуз\b)(?=.*\bмалина\b)(?=.*\bклубника\b).+$", RegexOptions.IgnoreCase);
...
string urlReplace = "<video src=\"$&\" controls width=\"640\" height=\"360\"></video>"; // $& — это всё совпадение

string htmlResult = Regex.Replace(text, urlPattern, urlReplace);
...
string replace = "(${area}) ${exchange}-${number}-${number2}";
...
Console.WriteLine("\nЗадача 10. Создать замещающий текст, который заместит совпадение с регулярным выражением текстом, расположенным перед совпадением, " +
    "всем испытуемым текстом и остатком испытуемого текста, расположенным после совпадения. " +
    "Например, если в тексте БаянУпражнениеШампур был найден фрагмент Упражнение, его следует заменить текстом ЛекцияУпражнениеЛабораторная, в результате " +
    "должен получиться текст БаянЛекцияУпражнениеЛабораторнаяШампур.");
